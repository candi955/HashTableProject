...
# of 100,000 random hash table entries, deletions, and removals
# Initially, a menu shows (menu() function at bottom of program) to offer the user choices

# Of special note: the time duration to complete 100,000 insert iteration of the hash and salt and place them in the
# excel file, for this project, estimated at approximately 1.34 seconds (for 9,378 hash/salt iterations for
# the run of 10,000 hash/salts on the hash1000Page.py, it took approximately 219 minutes, or 12,600 seconds to run).
...
class Hashing:
    ...
    def newNum(self):
        ...
        rand100000 = random.sample(range(1, 100001), 1)

        randomUpdateNum = random.sample(range(1, 255), 5)
        randomUpdateNumBytes = bytes(randomUpdateNum)

        dataRandom = np.append(dataEmpty, [rand100000])
        print("\nPlacing 100,000 random numbers into the dataset:\n", dataRandom)
        ...
        for item in dataRandomBytes:
            ...
            print("\nAdding a hash to the 100,000 dataset numbers:\n", itemHashed_and_Salted)

            k = itemHashed_and_Salted
            my_hash_table = {k: dataRandom}

            myNewDataframe = pd.DataFrame.from_dict(my_hash_table, orient='index')
            writer = pd.ExcelWriter('MyHashTable100_000.xlsx', engine='openpyxl')
            writer.book = load_workbook('MyHashTable100_000.xlsx')
            writer.sheets = dict((ws.title, ws) for ws in writer.book.worksheets)
            reader = pd.read_excel(r'MyHashTable100_000.xlsx')
    ...
    def newNumPrint(self):
        ...
        # The hashes were all showing up as same hash and salt, so created the function and now calling the function 100,000 times
        # reference: https://stackoverflow.com/questions/4264634/more-pythonic-way-to-run-a-process-x-times
        # the range (100000) at the current setup within this python version and IDE, is creating 100,000 hashes
        ...
        print("\n100,000 values have been added.\n")
        print("\nThe timing it took to add 100,000 random values with hashes/salt to the excel file is",
    ...
    def _getInfo_(self):
        ...
        MyDataframe = pd.read_excel('MyHashTable100_000.xlsx', sheet_name="Sheet1",
        ...
        if a is not None:
            ...
            print('Printing the requested hashed key and assigned random 100,000 number value:\n', keyRow, "\n")
        ...
        if deleteRequ == "Yes":
            ...
            if doubleChecking == "Yes":
                ...
                # Creating a variable for data being pulled from the MyEmployeeHashTable100_000.xlsx file and placed into array format
                book = xlrd.open_workbook('MyHashTable100_000.xlsx')
                sheets = book.sheets()
                for sheet in sheets:
                ...
                writer.book = load_workbook('MyHashTable100_000.xlsx')
                writer.sheets = dict((ws.title, ws) for ws in writer.book.worksheets)
                reader = pd.read_excel(r'MyHashTable100_000.xlsx', index_col=0)
                reader.drop(retrieveInput, axis=0, inplace=True)
                reader.to_excel('MyHashTable100_000.xlsx')  ##, index=True, header=False, startrow=len(reader)-1)
                writer.close()
        ...
        if deleteRequ == "Yes":
            ...
            if doubleChecking == "Yes":
                ...
                if reader.empty == False:
                    ...
                    reader.to_excel('MyHashTable100_000.xlsx')
                    writer.close()

                    print("\nThe file has been permanently deleted.\n")

                    print("\nThe timing it took to delete 100,000 random values with hashes/salt from the excel file is",
...
def main():
    ...
    asking = input("Please make a choice.\n"+
                   "1 to make an entry of 100,000 numbers onto the excel file\n" +
